  ...
  try {
    ...
    if (acceptHeader.includes("text/html")) {
      const htmlResponse = data
        .map(
          (product) =>
            `<div>ID: ${product.id}, Name: ${product.name}, Type: ${product.type}, Quality: ${product.quality}</div>`
        )
        .join("");
      ...
      const xmlResponse = `<data>${data
        .map(
          (product) =>
            `<product><id>${product.id}</id><name>${product.name}</name><type>${product.type}</type><quality>${product.quality}</quality></product>`
        )
        .join("")}</data>`;
